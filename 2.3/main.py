...
var: dict = {'front':[], # display in front of the decimal point with all chars separate
            'back':[], # display behind the decimal point with all chars separate
            'decimal':False, # Is the decimal point active or not.
            'x_val':0.0, # the consolidated value ahead of the operator 
            'y_val':0.0, # the consolidated value behind the operator
            'result':0.0, # the result of the operation
...
def calculate_click():
    ...
    try:
        var['result'] = eval(str(var['x_val']) + var['operator'] + str(var['y_val'])) # eval is safe here as the user can in no circumstances input textual data into this 
